...
### ✅ Communautés privées & modération (LIVRÉ — 2026-06-22)
`Community.visibility` (public/privé, modifiable par l'owner), `Membership.role` (owner/moderator/member), entité `CommunityRequest{kind:request|invite}` (repos memory+mongo, index unique du couple). Rejoindre une privée crée une **demande** (`joinCommunity`) ; owner/mods **approuvent/refusent** (`approveJoinRequest`/`rejectJoinRequest`) ; owner/mods **invitent par username** (`inviteToCommunity`) et l'invité **accepte/refuse** (`acceptInvite`/`declineInvite`) ; l'owner **promeut/rétrograde** (`setMemberRole`) et bascule la **visibilité** (`setCommunityVisibility`). **Fil masqué** aux non-membres d'une privée (`communityFeed` → `ForbiddenError`) ; vitrine visible en découverte. DTO enrichi (`visibility`/`role`/`requestState`/`canModerate`/`pendingCount`) + `listMembers`/`listJoinRequests`/`myInvites`. Routes `/api/communities/:id/{requests,requests/:reqId,invite,members,members/:userId/role,visibility}` + `/api/community-invites/{mine,:reqId}`. UI : toggle privé + cadenas + « Demander » (découverte) ; page communauté avec bandeau d'invitation, panneau de modération, liste des membres + rôles, toggle visibilité. Création toujours réservée à vulu+. Spec/plan : `docs/superpowers/{specs,plans}/2026-06-21-communautes-privees*`. **Différé** : notifications de demandes/invitations, transfert d'ownership.
